...
export default function RequestedFun() {
  ...
  const columns = [
    ...
    {
      title: <span className="text-[1vw]">Credit Period (Days)</span>,
      key: "credit_period",
      align: "center",
      render: (row) => {
        console.log(row, "741852");
            ...
            <span
              className={`flex border-[0.1vw] ${
                row.voi_advance_request_id === 0
                  ...
                  : "border-[#a5caec] bg-[#dff4ff] cursor-not-allowed"
  ...
  useEffect(() => {
    if (user_id === "VOIJ001") {
      ...
      Get_Voijeans_Status_By_Id(dispatch, currentTab, "advreq");
    }
  }, [currentTab]);
  return (
    <>
